...
class TestUtility:
    ...
    def test_get_image_returns_image(self, desired_dimensions: Tuple[int, int]):
        img_path = str(IMAGE_NOT_FOUND_PATH)
        image = utility.get_image(img_path, desired_dimensions)
    ...
    def test_flatten_images(self):
        ...
        image = utility.flatten_images(images)

        assert isinstance(image, pygame.Surface)

    def test_build_sprites_from_paths(self):
        ...
        trait_sprites = utility.build_sprites_from_paths(_trait_paths, (TILE_SIZE, TILE_SIZE))
    ...
    def test_clamp(self, value: Union[int, float], min_value: Union[int, float], max_value: Union[int, float]):
    ...
    def test_is_close(self, current_pos: Tuple[float, float], target_pos: Tuple[float, float], delta: float):
    ...
    def test_is_not_close(self, current_pos: Tuple[float, float], target_pos: Tuple[float, float], delta: float):
        result = utility.is_close(current_pos, target_pos, delta)
