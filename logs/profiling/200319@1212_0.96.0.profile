Thu Mar 19 12:12:01 2020    logs/profiling/profile.dump

         2298622 function calls (2246006 primitive calls) in 19.458 seconds

   Ordered by: cumulative time

   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
        1    0.212    0.212   19.411   19.411 main.py:78(game_loop)
     1152   13.198    0.011   13.198    0.011 {method 'tick' of 'Clock' objects}
      576    0.003    0.000    6.725    0.012 state.py:63(update_clock)
      576    0.004    0.000    6.480    0.011 state.py:38(get_delta_time)
      576    0.010    0.000    2.744    0.005 manager.py:73(draw)
      576    0.003    0.000    2.628    0.005 manager.py:54(update)
      576    0.168    0.000    2.626    0.005 ui_manager.py:122(update)
   185674    1.941    0.000    1.941    0.000 {method 'blit' of 'pygame.Surface' objects}
      576    0.090    0.000    1.565    0.003 sprite.py:453(update)
      575    0.004    0.000    1.153    0.002 camera.py:72(update)
      577    0.552    0.001    1.152    0.002 camera.py:79(update_game_map)
      576    0.005    0.000    1.042    0.002 ui_manager.py:173(draw_ui)
      576    0.169    0.000    1.038    0.002 sprite.py:753(draw)
      580    0.781    0.001    0.781    0.001 {built-in method pygame.transform.scale}
    90458    0.387    0.000    0.689    0.000 ui_element.py:121(check_hover)
      576    0.001    0.000    0.350    0.001 event_core.py:21(update)
        7    0.000    0.000    0.338    0.048 ui_handler.py:30(process_event)
        2    0.000    0.000    0.325    0.162 ui_handler.py:207(update_camera)
        2    0.000    0.000    0.320    0.160 manager.py:295(update_camera_grid)
        2    0.002    0.001    0.320    0.160 camera.py:106(update_grid)
      305    0.004    0.000    0.317    0.001 ui_button.py:30(__init__)
      305    0.016    0.000    0.298    0.001 ui_button.py:403(rebuild_from_changed_theme_data)
    89125    0.174    0.000    0.283    0.000 ui_button.py:197(update)
     8890    0.022    0.000    0.240    0.000 ui_appearance_theme.py:347(build_all_combined_ids)
      576    0.216    0.000    0.216    0.000 {built-in method pygame.event.get}
61470/8890    0.204    0.000    0.216    0.000 ui_appearance_theme.py:322(get_next_id_node)
      576    0.209    0.000    0.209    0.000 {built-in method pygame.display.flip}
    89125    0.107    0.000    0.208    0.000 ui_button.py:138(hover_point)
     5861    0.179    0.000    0.189    0.000 sprite.py:913(get_sprites_from_layer)
        6    0.000    0.000    0.174    0.029 ui_handler.py:72(process_game_event)
        1    0.000    0.000    0.169    0.169 ui_handler.py:111(init_game_ui)
        1    0.000    0.000    0.164    0.164 ui_handler.py:48(process_entity_event)
     4596    0.012    0.000    0.135    0.000 ui_appearance_theme.py:466(get_colour_or_gradient)
    86553    0.125    0.000    0.125    0.000 camera.py:234(world_to_screen_position)
    89125    0.089    0.000    0.101    0.000 rect_drawable_shape.py:84(collide_point)
      841    0.082    0.000    0.082    0.000 {method 'fill' of 'pygame.Surface' objects}
     2766    0.005    0.000    0.079    0.000 ui_appearance_theme.py:428(get_misc_data)
   183792    0.063    0.000    0.076    0.000 sprite.py:208(alive)
    89125    0.046    0.000    0.075    0.000 drawable_shape.py:36(update)
        1    0.000    0.000    0.047    0.047 main.py:183(initialise_game)
        2    0.000    0.000    0.043    0.022 entity.py:225(create_actor)
       65    0.000    0.000    0.042    0.001 manager.py:60(process_ui_events)
       65    0.015    0.000    0.041    0.001 ui_manager.py:86(process_events)
    89125    0.037    0.000    0.037    0.000 ui_button.py:154(can_hover)
      305    0.002    0.000    0.037    0.000 ui_button.py:97(set_any_images_from_theme)
        2    0.009    0.004    0.036    0.018 world.py:26(create_fov_map)
     1220    0.002    0.000    0.035    0.000 ui_appearance_theme.py:366(get_image)
     1528    0.010    0.000    0.031    0.000 rect_drawable_shape.py:118(redraw_state)
   317214    0.029    0.000    0.029    0.000 {method 'append' of 'list' objects}
      183    0.001    0.000    0.022    0.000 screen_message.py:34(update)
     4503    0.006    0.000    0.020    0.000 _internal.py:24(wrapper)
      758    0.006    0.000    0.020    0.000 ui_text_box.py:205(update)
      305    0.003    0.000    0.019    0.000 ui_button.py:537(rebuild_shape)
      123    0.000    0.000    0.019    0.000 ui_text_box.py:347(redraw_from_chunks)
      308    0.001    0.000    0.017    0.000 rect_drawable_shape.py:22(__init__)
      308    0.005    0.000    0.015    0.000 rect_drawable_shape.py:32(full_rebuild_on_size_change)
    94667    0.015    0.000    0.015    0.000 ui_manager.py:167(get_mouse_position)
      318    0.003    0.000    0.015    0.000 ui_element.py:23(__init__)
     3451    0.011    0.000    0.014    0.000 ui_container.py:124(check_hover)
    93909    0.014    0.000    0.014    0.000 {method 'collidepoint' of 'pygame.Rect' objects}
     3304    0.008    0.000    0.013    0.000 world.py:55(get_tile)
   183792    0.013    0.000    0.013    0.000 {built-in method _operator.truth}
      123    0.002    0.000    0.013    0.000 ui_text_box.py:327(redraw_from_text_block)
      305    0.001    0.000    0.012    0.000 ui_appearance_theme.py:405(get_font)
    90307    0.012    0.000    0.012    0.000 {method 'union' of 'pygame.Rect' objects}
   190634    0.011    0.000    0.011    0.000 {built-in method builtins.len}
    96678    0.010    0.000    0.010    0.000 {method 'colliderect' of 'pygame.Rect' objects}
     4504    0.009    0.000    0.010    0.000 {built-in method _warnings.warn}
     1546    0.010    0.000    0.010    0.000 {method 'copy' of 'pygame.Surface' objects}
      308    0.001    0.000    0.009    0.000 drawable_shape.py:45(redraw_all_states)
      576    0.002    0.000    0.009    0.000 processors.py:16(process_all)
        7    0.000    0.000    0.009    0.001 entity_handler.py:29(process_event)
        1    0.000    0.000    0.008    0.008 entity_handler.py:57(_process_move)
     1528    0.008    0.000    0.008    0.000 surface_cache.py:119(build_cache_id)
        1    0.002    0.002    0.008    0.008 world.py:445(update_tile_visibility)
        3    0.000    0.000    0.007    0.002 ui_text_box.py:50(__init__)
        3    0.000    0.000    0.007    0.002 ui_text_box.py:492(rebuild_from_changed_theme_data)
      318    0.001    0.000    0.007    0.000 ui_container.py:42(add_element)
      576    0.007    0.000    0.007    0.000 processors.py:23(_process_aesthetic_update)
        3    0.000    0.000    0.007    0.002 ui_text_box.py:110(rebuild)
       62    0.000    0.000    0.007    0.000 ui_text_box.py:462(set_active_effect)
     2876    0.005    0.000    0.007    0.000 ui_window.py:97(update)
      641    0.004    0.000    0.006    0.000 sprite.py:814(layers)
        2    0.000    0.000    0.006    0.003 entity.py:332(build_characteristic_sprites)
       40    0.000    0.000    0.006    0.000 utility.py:13(get_image)
     7905    0.006    0.000    0.006    0.000 ui_button.py:257(process_event)
        3    0.000    0.000    0.006    0.002 ui_text_box.py:310(parse_html_into_style_data)
      123    0.002    0.000    0.005    0.000 text_block.py:265(redraw_from_chunks)
       12    0.000    0.000    0.005    0.000 utility.py:39(get_images)
        3    0.000    0.000    0.005    0.002 text_block.py:16(__init__)
        3    0.000    0.000    0.005    0.002 text_block.py:40(redraw)
       41    0.005    0.000    0.005    0.000 {built-in method pygame.imageext.load_extended}
        1    0.000    0.000    0.005    0.005 manager.py:223(create_screen_message)
        1    0.000    0.000    0.005    0.005 screen_message.py:16(__init__)
     1152    0.005    0.000    0.005    0.000 sprite.py:745(sprites)
     3305    0.004    0.000    0.005    0.000 world.py:347(_is_tile_in_bounds)
       36    0.000    0.000    0.005    0.000 __init__.py:1496(_log)
      318    0.001    0.000    0.004    0.000 sprite.py:121(__init__)
       32    0.000    0.000    0.004    0.000 __init__.py:1996(debug)
      470    0.004    0.000    0.004    0.000 ui_container.py:62(recalculate_container_layer_thickness)
       32    0.000    0.000    0.004    0.000 __init__.py:1361(debug)
      576    0.003    0.000    0.004    0.000 ui_manager.py:158(update_mouse_position)
      312    0.001    0.000    0.004    0.000 ui_font_dictionary.py:89(find_font)
        1    0.000    0.000    0.004    0.004 manager.py:182(init_skill_bar)
        1    0.000    0.000    0.004    0.004 skill_bar.py:15(__init__)
      576    0.002    0.000    0.004    0.000 ui_appearance_theme.py:158(update_shape_cache)
      502    0.004    0.000    0.004    0.000 ui_manager.py:104(<listcomp>)
      318    0.001    0.000    0.004    0.000 sprite.py:126(add)
        1    0.000    0.000    0.004    0.004 world.py:19(create_game_map)
        1    0.002    0.002    0.004    0.004 game_map.py:12(__init__)
        2    0.001    0.000    0.004    0.002 ui_container.py:116(clear)
        2    0.000    0.000    0.003    0.002 manager.py:286(update_camera_game_map)
      575    0.002    0.000    0.003    0.000 skill_bar.py:45(update)
     3000    0.002    0.000    0.003    0.000 libtcodpy.py:3254(map_set_properties)
        1    0.000    0.000    0.003    0.003 manager.py:156(init_message_log)
        1    0.000    0.000    0.003    0.003 message_log.py:18(__init__)
      318    0.001    0.000    0.003    0.000 ui_element.py:104(change_layer)
      150    0.000    0.000    0.003    0.000 ui_button.py:130(kill)
     1282    0.003    0.000    0.003    0.000 {built-in method builtins.sorted}
      152    0.000    0.000    0.003    0.000 ui_element.py:114(kill)
      575    0.001    0.000    0.003    0.000 message_log.py:36(update)
     1528    0.002    0.000    0.002    0.000 drawable_shape.py:122(rebuild_images_and_text)
        1    0.002    0.002    0.002    0.002 ui_font_dictionary.py:155(preload_font)
       36    0.000    0.000    0.002    0.000 __init__.py:1521(handle)
      326    0.002    0.000    0.002    0.000 sprite.py:822(change_layer)
      318    0.002    0.000    0.002    0.000 sprite.py:646(add_internal)
      576    0.001    0.000    0.002    0.000 surface_cache.py:24(update)
       36    0.000    0.000    0.002    0.000 __init__.py:1575(callHandlers)
    35180    0.002    0.000    0.002    0.000 {method 'reverse' of 'list' objects}
       36    0.000    0.000    0.002    0.000 __init__.py:892(handle)
        6    0.000    0.000    0.002    0.000 game_handler.py:26(process_event)
     1333    0.001    0.000    0.002    0.000 ui_element.py:186(hover_point)
        1    0.000    0.000    0.002    0.002 message_log.py:49(add_message)
       36    0.000    0.000    0.002    0.000 __init__.py:1123(emit)
      575    0.001    0.000    0.002    0.000 entity_info.py:45(update)
       36    0.000    0.000    0.002    0.000 __init__.py:1022(emit)
     1159    0.002    0.000    0.002    0.000 state.py:45(get_current)
      152    0.000    0.000    0.002    0.000 ui_container.py:52(remove_element)
       36    0.000    0.000    0.001    0.000 __init__.py:1481(makeRecord)
        2    0.000    0.000    0.001    0.001 manager.py:275(update_cameras_tiles)
        2    0.000    0.000    0.001    0.001 camera.py:168(update_camera_tiles)
     3206    0.001    0.000    0.001    0.000 ui_window.py:107(get_container)
     3000    0.001    0.000    0.001    0.000 {built-in method tcod._libtcod.TCOD_map_set_properties}
       36    0.000    0.000    0.001    0.000 __init__.py:293(__init__)
      576    0.001    0.000    0.001    0.000 {built-in method pygame.mouse.get_pos}
     6612    0.001    0.000    0.001    0.000 world.py:48(get_game_map)
       27    0.001    0.000    0.001    0.000 {method 'render' of 'pygame.font.Font' objects}
       65    0.000    0.000    0.001    0.000 processors.py:57(process_intent)
     1500    0.001    0.000    0.001    0.000 libtcodpy.py:3300(map_is_in_fov)
        2    0.000    0.000    0.001    0.001 styled_chunk.py:8(__init__)
     1500    0.001    0.000    0.001    0.000 tile.py:20(__init__)
        1    0.000    0.000    0.001    0.001 manager.py:195(init_camera)
        1    0.001    0.001    0.001    0.001 camera.py:24(__init__)
       36    0.000    0.000    0.001    0.000 __init__.py:869(format)
        4    0.001    0.000    0.001    0.000 {method 'metrics' of 'pygame.font.Font' objects}
       36    0.000    0.000    0.001    0.000 __init__.py:606(format)
       51    0.000    0.000    0.001    0.000 processors.py:138(_process_player_turn_intents)
      308    0.001    0.000    0.001    0.000 drawable_shape.py:11(__init__)
     1528    0.001    0.000    0.001    0.000 surface_cache.py:109(find_surface_in_cache)
       10    0.001    0.000    0.001    0.000 {built-in method nt.stat}
      152    0.000    0.000    0.001    0.000 sprite.py:183(kill)
        9    0.000    0.000    0.001    0.000 ui_appearance_theme.py:138(check_need_to_reload)
      314    0.001    0.000    0.001    0.000 ui_element.py:68(create_valid_ids)
       65    0.000    0.000    0.001    0.000 action.py:12(convert_to_intent)
        9    0.000    0.000    0.001    0.000 surface_cache.py:29(add_surface_to_long_term_cache)
     4026    0.001    0.000    0.001    0.000 sprite.py:168(update)
      576    0.001    0.000    0.001    0.000 {built-in method builtins.any}
        6    0.000    0.000    0.001    0.000 dataclasses.py:1023(asdict)
      308    0.000    0.000    0.001    0.000 drawable_shape.py:50(compute_aligned_text_rect)
     42/6    0.000    0.000    0.001    0.000 dataclasses.py:1047(_asdict_inner)
        4    0.000    0.000    0.001    0.000 game_handler.py:42(process_change_game_state)
       36    0.000    0.000    0.001    0.000 __init__.py:1011(flush)
        1    0.000    0.000    0.001    0.001 game_handler.py:81(process_end_turn)
        1    0.000    0.000    0.001    0.001 chrono.py:50(next_turn)
     2876    0.001    0.000    0.001    0.000 ui_window.py:116(check_hover)
        4    0.000    0.000    0.001    0.000 ui_window.py:18(__init__)
       38    0.000    0.000    0.001    0.000 ntpath.py:212(basename)
     1500    0.001    0.000    0.001    0.000 {built-in method tcod._libtcod.TCOD_map_is_in_fov}
        1    0.000    0.000    0.001    0.001 warnings.py:96(_showwarnmsg)
      152    0.000    0.000    0.001    0.000 sprite.py:728(remove_internal)
        1    0.000    0.000    0.001    0.001 warnings.py:20(_showwarnmsg_impl)
     3722    0.000    0.000    0.000    0.000 {built-in method builtins.isinstance}
       38    0.000    0.000    0.000    0.000 ntpath.py:178(split)
       36    0.000    0.000    0.000    0.000 __init__.py:1451(findCaller)
        4    0.000    0.000    0.000    0.000 __init__.py:1986(info)
        4    0.000    0.000    0.000    0.000 __init__.py:1373(info)
      313    0.000    0.000    0.000    0.000 ui_font_dictionary.py:133(create_font_id)
       36    0.000    0.000    0.000    0.000 __init__.py:539(formatTime)
        3    0.000    0.000    0.000    0.000 state.py:71(set_new)
        1    0.000    0.000    0.000    0.000 entity.py:194(create_god)
       35    0.000    0.000    0.000    0.000 {method 'convert_alpha' of 'pygame.Surface' objects}
     1232    0.000    0.000    0.000    0.000 {built-in method math.floor}
       37    0.000    0.000    0.000    0.000 {method 'write' of '_io.TextIOWrapper' objects}
       36    0.000    0.000    0.000    0.000 {method 'flush' of '_io.TextIOWrapper' objects}
     1333    0.000    0.000    0.000    0.000 ui_element.py:204(can_hover)
        3    0.000    0.000    0.000    0.000 parser.py:104(feed)
        3    0.000    0.000    0.000    0.000 parser.py:134(goahead)
      380    0.000    0.000    0.000    0.000 ui_window_stack.py:73(get_root_window)
        1    0.000    0.000    0.000    0.000 warnings.py:117(_formatwarnmsg)
        1    0.000    0.000    0.000    0.000 warnings.py:35(_formatwarnmsg_impl)
      494    0.000    0.000    0.000    0.000 {method 'remove' of 'list' objects}
        1    0.000    0.000    0.000    0.000 linecache.py:15(getline)
        1    0.000    0.000    0.000    0.000 linecache.py:37(getlines)
       11    0.000    0.000    0.000    0.000 {method 'size' of 'pygame.font.Font' objects}
        1    0.000    0.000    0.000    0.000 linecache.py:82(updatecache)
       44    0.000    0.000    0.000    0.000 {built-in method pygame.transform.smoothscale}
      712    0.000    0.000    0.000    0.000 {method 'values' of 'dict' objects}
        1    0.000    0.000    0.000    0.000 chrono.py:23(rebuild_turn_queue)
        1    0.000    0.000    0.000    0.000 manager.py:169(init_entity_info)
       20    0.000    0.000    0.000    0.000 entity.py:123(get_primary_stat)
        1    0.000    0.000    0.000    0.000 entity_info.py:19(__init__)
       36    0.000    0.000    0.000    0.000 ntpath.py:201(splitext)
        5    0.000    0.000    0.000    0.000 ui_container.py:19(__init__)
      308    0.000    0.000    0.000    0.000 drawable_shape.py:46(<listcomp>)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:555(get_abs_path_real_path_and_base_from_file)
     1528    0.000    0.000    0.000    0.000 {method 'popleft' of 'collections.deque' objects}
       36    0.000    0.000    0.000    0.000 {built-in method time.strftime}
        3    0.000    0.000    0.000    0.000 html_parser.py:207(__init__)
       62    0.000    0.000    0.000    0.000 text_effects.py:81(__init__)
        3    0.000    0.000    0.000    0.000 html_parser.py:60(__init__)
        2    0.000    0.000    0.000    0.000 combat_stats.py:67(max_health)
        1    0.000    0.000    0.000    0.000 god_handler.py:26(process_event)
      988    0.000    0.000    0.000    0.000 {built-in method builtins.hasattr}
      332    0.000    0.000    0.000    0.000 drawable_shape.py:86(get_surface)
       51    0.000    0.000    0.000    0.000 processors.py:71(_get_pressed_direction)
       61    0.000    0.000    0.000    0.000 text_effects.py:88(update)
      644    0.000    0.000    0.000    0.000 {method 'insert' of 'list' objects}
      626    0.000    0.000    0.000    0.000 {built-in method builtins.min}
       76    0.000    0.000    0.000    0.000 ntpath.py:44(normcase)
        3    0.000    0.000    0.000    0.000 entity.py:166(create)
      616    0.000    0.000    0.000    0.000 {built-in method builtins.max}
      269    0.000    0.000    0.000    0.000 ui_window.py:55(process_event)
       55    0.000    0.000    0.000    0.000 entity.py:37(get_player)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:228(_NormPaths)
       34    0.000    0.000    0.000    0.000 entity.py:325(add_component)
       42    0.000    0.000    0.000    0.000 ntpath.py:122(splitdrive)
        9    0.000    0.000    0.000    0.000 surface_cache.py:21(add_surface_to_cache)
        2    0.000    0.000    0.000    0.000 combat_stats.py:92(max_stamina)
        4    0.000    0.000    0.000    0.000 pydevd_file_utils.py:245(_NormPath)
       80    0.000    0.000    0.000    0.000 ui_text_box.py:379(process_event)
       36    0.000    0.000    0.000    0.000 genericpath.py:117(_splitext)
       65    0.000    0.000    0.000    0.000 action.py:34(_check_directions)
      645    0.000    0.000    0.000    0.000 ui_manager.py:44(get_sprite_group)
       96    0.000    0.000    0.000    0.000 ui_button.py:170(while_hovering)
       12    0.000    0.000    0.000    0.000 utility.py:51(flatten_images)
        5    0.000    0.000    0.000    0.000 ui_manager.py:59(get_shadow)
      371    0.000    0.000    0.000    0.000 ui_element.py:210(process_event)
       34    0.000    0.000    0.000    0.000 esper.py:196(add_component)
        4    0.000    0.000    0.000    0.000 ntpath.py:523(abspath)
       36    0.000    0.000    0.000    0.000 copy.py:132(deepcopy)
        2    0.000    0.000    0.000    0.000 libtcodpy.py:3228(map_new)
       65    0.000    0.000    0.000    0.000 processors.py:118(_process_stateless_intents)
        2    0.000    0.000    0.000    0.000 parser.py:301(parse_starttag)
      318    0.000    0.000    0.000    0.000 sprite.py:162(add_internal)
        2    0.000    0.000    0.000    0.000 map.py:66(__init__)
       36    0.000    0.000    0.000    0.000 __init__.py:590(formatMessage)
        1    0.000    0.000    0.000    0.000 tokenize.py:443(open)
        5    0.000    0.000    0.000    0.000 ui_shadow.py:178(find_closest_shadow_scale_to_size)
       51    0.000    0.000    0.000    0.000 processors.py:98(_get_pressed_skills_number)
      612    0.000    0.000    0.000    0.000 {method 'copy' of 'list' objects}
        4    0.000    0.000    0.000    0.000 ntpath.py:450(normpath)
       36    0.000    0.000    0.000    0.000 {built-in method time.gmtime}
       14    0.000    0.000    0.000    0.000 surface_cache.py:80(split_rect)
      326    0.000    0.000    0.000    0.000 {method 'pop' of 'dict' objects}
      128    0.000    0.000    0.000    0.000 {method 'set_alpha' of 'pygame.Surface' objects}
        1    0.000    0.000    0.000    0.000 {method 'readlines' of '_io._IOBase' objects}
       36    0.000    0.000    0.000    0.000 __init__.py:584(usesTime)
        4    0.000    0.000    0.000    0.000 ui_window_stack.py:23(add_new_window)
       36    0.000    0.000    0.000    0.000 cp1252.py:18(encode)
        4    0.000    0.000    0.000    0.000 combat_stats.py:22(vigour)
        7    0.000    0.000    0.000    0.000 utility.py:188(value_to_member)
       72    0.000    0.000    0.000    0.000 __init__.py:849(acquire)
        2    0.000    0.000    0.000    0.000 map.py:74(__as_cdata)
        4    0.000    0.000    0.000    0.000 ui_window.py:135(change_window_layer)
       36    0.000    0.000    0.000    0.000 __init__.py:432(format)
      318    0.000    0.000    0.000    0.000 {method '__contains__' of 'dict' objects}
        6    0.000    0.000    0.000    0.000 dataclasses.py:994(fields)
      319    0.000    0.000    0.000    0.000 ui_manager.py:51(get_window_stack)
        4    0.000    0.000    0.000    0.000 combat_stats.py:31(clout)
      318    0.000    0.000    0.000    0.000 ui_manager.py:37(get_theme)
        2    0.000    0.000    0.000    0.000 world.py:260(tile_has_tag)
        4    0.000    0.000    0.000    0.000 combat_stats.py:40(skullduggery)
        3    0.000    0.000    0.000    0.000 esper.py:274(get_components)
        4    0.000    0.000    0.000    0.000 combat_stats.py:49(bustle)
        4    0.000    0.000    0.000    0.000 combat_stats.py:58(exactitude)
        1    0.000    0.000    0.000    0.000 ui_image.py:20(__init__)
       64    0.000    0.000    0.000    0.000 action.py:60(_check_actions)
      307    0.000    0.000    0.000    0.000 {method 'copy' of 'pygame.Rect' objects}
        7    0.000    0.000    0.000    0.000 utility.py:94(get_class_members)
        1    0.000    0.000    0.000    0.000 {built-in method io.open}
       72    0.000    0.000    0.000    0.000 __init__.py:856(release)
        1    0.000    0.000    0.000    0.000 main.py:210(initialise_event_handlers)
        3    0.000    0.000    0.000    0.000 html_parser.py:118(add_text)
       13    0.000    0.000    0.000    0.000 ui_button.py:162(on_hovered)
        2    0.000    0.000    0.000    0.000 ui_text_box.py:102(kill)
      305    0.000    0.000    0.000    0.000 drawable_shape.py:33(clean_up_temp_shapes)
       63    0.000    0.000    0.000    0.000 action.py:83(_check_dev_actions)
       36    0.000    0.000    0.000    0.000 __init__.py:429(usesTime)
       48    0.000    0.000    0.000    0.000 dataclasses.py:1012(_is_dataclass_instance)
        3    0.000    0.000    0.000    0.000 esper.py:276(<listcomp>)
        1    0.000    0.000    0.000    0.000 world.py:438(recompute_fov)
       36    0.000    0.000    0.000    0.000 __init__.py:1619(isEnabledFor)
        9    0.000    0.000    0.000    0.000 {method 'subsurface' of 'pygame.Surface' objects}
        3    0.000    0.000    0.000    0.000 ui_appearance_theme.py:386(get_font_info)
       11    0.000    0.000    0.000    0.000 entity.py:86(get_entitys_component)
       36    0.000    0.000    0.000    0.000 __init__.py:154(<lambda>)
        2    0.000    0.000    0.000    0.000 html_parser.py:213(handle_starttag)
       72    0.000    0.000    0.000    0.000 __init__.py:747(filter)
        3    0.000    0.000    0.000    0.000 html_parser.py:123(add_indexed_style)
        2    0.000    0.000    0.000    0.000 html_parser.py:283(handle_data)
       62    0.000    0.000    0.000    0.000 text_effects.py:2(__init__)
        4    0.000    0.000    0.000    0.000 event.py:106(__init__)
       36    0.000    0.000    0.000    0.000 {built-in method _codecs.charmap_encode}
       34    0.000    0.000    0.000    0.000 esper.py:51(clear_cache)
      171    0.000    0.000    0.000    0.000 {method 'pop' of 'list' objects}
        4    0.000    0.000    0.000    0.000 esper.py:270(get_component)
       11    0.000    0.000    0.000    0.000 ui_button.py:187(on_unhovered)
       36    0.000    0.000    0.000    0.000 __init__.py:117(getLevelName)
        1    0.000    0.000    0.000    0.000 tokenize.py:350(detect_encoding)
        4    0.000    0.000    0.000    0.000 entity.py:96(get_name)
      108    0.000    0.000    0.000    0.000 {method 'rfind' of 'str' objects}
       36    0.000    0.000    0.000    0.000 threading.py:1206(current_thread)
        1    0.000    0.000    0.000    0.000 ui_shadow.py:99(create_new_rectangle_shadow)
      120    0.000    0.000    0.000    0.000 esper.py:278(try_component)
      169    0.000    0.000    0.000    0.000 {method 'get' of 'dict' objects}
        9    0.000    0.000    0.000    0.000 esper.py:254(_get_components)
       61    0.000    0.000    0.000    0.000 text_effects.py:100(should_redraw_from_chunks)
        1    0.000    0.000    0.000    0.000 world.py:359(_is_tile_blocking_movement)
      132    0.000    0.000    0.000    0.000 {method 'lower' of 'str' objects}
      122    0.000    0.000    0.000    0.000 {method 'replace' of 'str' objects}
      196    0.000    0.000    0.000    0.000 {built-in method nt.fspath}
      346    0.000    0.000    0.000    0.000 {method 'contains' of 'pygame.Rect' objects}
        4    0.000    0.000    0.000    0.000 ui_container.py:75(change_container_layer)
       36    0.000    0.000    0.000    0.000 __init__.py:371(getMessage)
       15    0.000    0.000    0.000    0.000 {method 'match' of 're.Pattern' objects}
      123    0.000    0.000    0.000    0.000 text_effects.py:107(get_final_alpha)
        7    0.000    0.000    0.000    0.000 event_core.py:38(publish)
        8    0.000    0.000    0.000    0.000 html_parser.py:94(push_style)
        4    0.000    0.000    0.000    0.000 esper.py:272(<listcomp>)
       74    0.000    0.000    0.000    0.000 {method 'acquire' of '_thread.RLock' objects}
        6    0.000    0.000    0.000    0.000 utility.py:107(lerp)
        4    0.000    0.000    0.000    0.000 entity.py:109(get_identity)
       38    0.000    0.000    0.000    0.000 ntpath.py:34(_get_bothseps)
       47    0.000    0.000    0.000    0.000 {method 'find' of 'str' objects}
       36    0.000    0.000    0.000    0.000 threading.py:1052(name)
      138    0.000    0.000    0.000    0.000 {built-in method builtins.getattr}
        1    0.000    0.000    0.000    0.000 libtcodpy.py:3283(map_compute_fov)
        3    0.000    0.000    0.000    0.000 manager.py:398(world_to_screen_position)
        1    0.000    0.000    0.000    0.000 parser.py:386(parse_endtag)
       61    0.000    0.000    0.000    0.000 text_effects.py:5(should_full_redraw)
        2    0.000    0.000    0.000    0.000 {method 'new' of 'CompiledFFI' objects}
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:550(is_real_file)
        1    0.000    0.000    0.000    0.000 map.py:100(compute_fov)
        8    0.000    0.000    0.000    0.000 event_core.py:50(subscribe)
        1    0.000    0.000    0.000    0.000 manager.py:345(should_camera_move)
        3    0.000    0.000    0.000    0.000 parser.py:87(__init__)
      152    0.000    0.000    0.000    0.000 {method 'clear' of 'dict' objects}
        2    0.000    0.000    0.000    0.000 parser.py:352(check_for_whole_start_tag)
       42    0.000    0.000    0.000    0.000 dataclasses.py:1009(<genexpr>)
        2    0.000    0.000    0.000    0.000 {built-in method numpy.zeros}
        1    0.000    0.000    0.000    0.000 world.py:395(_tile_has_other_entity)
        1    0.000    0.000    0.000    0.000 tokenize.py:374(read_or_stop)
        3    0.000    0.000    0.000    0.000 __init__.py:186(easeOutCubic)
        9    0.000    0.000    0.000    0.000 surface_cache.py:68(<listcomp>)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:542(_is_int)
        4    0.000    0.000    0.000    0.000 pydevd_file_utils.py:171(normcase)
       36    0.000    0.000    0.000    0.000 {built-in method sys._getframe}
        1    0.000    0.000    0.000    0.000 {method 'readline' of '_io.BufferedReader' objects}
        3    0.000    0.000    0.000    0.000 codecs.py:319(decode)
        1    0.000    0.000    0.000    0.000 {built-in method tcod._libtcod.TCOD_map_compute_fov}
        7    0.000    0.000    0.000    0.000 event_core.py:12(notify)
       38    0.000    0.000    0.000    0.000 {method 'rstrip' of 'str' objects}
       36    0.000    0.000    0.000    0.000 {built-in method time.time}
       36    0.000    0.000    0.000    0.000 {built-in method nt.getpid}
        3    0.000    0.000    0.000    0.000 parser.py:96(reset)
       72    0.000    0.000    0.000    0.000 {built-in method _thread.get_ident}
        6    0.000    0.000    0.000    0.000 _markupbase.py:48(updatepos)
       12    0.000    0.000    0.000    0.000 esper.py:243(_get_component)
        6    0.000    0.000    0.000    0.000 utility.py:121(clamp)
        8    0.000    0.000    0.000    0.000 event_core.py:15(subscribe)
       74    0.000    0.000    0.000    0.000 {method 'release' of '_thread.RLock' objects}
        1    0.000    0.000    0.000    0.000 entity_handler.py:236(_process_end_turn)
        1    0.000    0.000    0.000    0.000 html_parser.py:272(handle_endtag)
        2    0.000    0.000    0.000    0.000 _internal.py:251(__init__)
       13    0.000    0.000    0.000    0.000 esper.py:176(has_component)
        9    0.000    0.000    0.000    0.000 surface_cache.py:62(<listcomp>)
        3    0.000    0.000    0.000    0.000 {built-in method _codecs.utf_8_decode}
        9    0.000    0.000    0.000    0.000 {method 'get_size' of 'pygame.Surface' objects}
        7    0.000    0.000    0.000    0.000 event_core.py:62(__init__)
        2    0.000    0.000    0.000    0.000 entity.py:116(get_combat_stats)
       22    0.000    0.000    0.000    0.000 library.py:123(get_savvy_data)
       22    0.000    0.000    0.000    0.000 library.py:139(get_people_data)
        2    0.000    0.000    0.000    0.000 {method 'cast' of 'CompiledFFI' objects}
       68    0.000    0.000    0.000    0.000 {method 'cache_clear' of 'functools._lru_cache_wrapper' objects}
        4    0.000    0.000    0.000    0.000 {built-in method nt._getfullpathname}
       22    0.000    0.000    0.000    0.000 library.py:155(get_homeland_data)
        1    0.000    0.000    0.000    0.000 event.py:63(__init__)
        2    0.000    0.000    0.000    0.000 <string>:1(__init__)
       10    0.000    0.000    0.000    0.000 {method 'split' of 'str' objects}
       13    0.000    0.000    0.000    0.000 manager.py:128(get_ui_element)
        2    0.000    0.000    0.000    0.000 manager.py:236(is_target_pos_in_camera_edge)
        3    0.000    0.000    0.000    0.000 component.py:46(__init__)
       36    0.000    0.000    0.000    0.000 copy.py:190(_deepcopy_atomic)
        6    0.000    0.000    0.000    0.000 html_parser.py:8(__init__)
        1    0.000    0.000    0.000    0.000 event.py:98(__init__)
        3    0.000    0.000    0.000    0.000 {built-in method math.sin}
        1    0.000    0.000    0.000    0.000 entity.py:370(spend_time)
        1    0.000    0.000    0.000    0.000 manager.py:305(set_player_tile)
        1    0.000    0.000    0.000    0.000 main.py:160(disable_profiling)
        2    0.000    0.000    0.000    0.000 _internal.py:45(verify_order)
       43    0.000    0.000    0.000    0.000 {method 'keys' of 'dict' objects}
        2    0.000    0.000    0.000    0.000 __init__.py:212(_acquireLock)
        3    0.000    0.000    0.000    0.000 html_parser.py:27(__init__)
        3    0.000    0.000    0.000    0.000 esper.py:100(create_entity)
        6    0.000    0.000    0.000    0.000 esper.py:266(<listcomp>)
        2    0.000    0.000    0.000    0.000 styled_chunk.py:73(unset_underline_style)
       15    0.000    0.000    0.000    0.000 {method 'startswith' of 'str' objects}
       36    0.000    0.000    0.000    0.000 {built-in method builtins.id}
        5    0.000    0.000    0.000    0.000 event_core.py:46(__init__)
       11    0.000    0.000    0.000    0.000 esper.py:146(component_for_entity)
        1    0.000    0.000    0.000    0.000 game_handler.py:23(__init__)
       18    0.000    0.000    0.000    0.000 {method 'update' of 'dict' objects}
        1    0.000    0.000    0.000    0.000 tokenize.py:380(find_cookie)
        9    0.000    0.000    0.000    0.000 {method 'popitem' of 'dict' objects}
        1    0.000    0.000    0.000    0.000 event.py:88(__init__)
       34    0.000    0.000    0.000    0.000 {method 'add' of 'set' objects}
        3    0.000    0.000    0.000    0.000 text_block.py:288(add_chunks_to_hover_group)
        1    0.000    0.000    0.000    0.000 html_parser.py:102(pop_style)
        1    0.000    0.000    0.000    0.000 codecs.py:309(__init__)
        3    0.000    0.000    0.000    0.000 component.py:87(__init__)
        4    0.000    0.000    0.000    0.000 library.py:206(get_secondary_stat_data)
        7    0.000    0.000    0.000    0.000 {method 'keys' of 'mappingproxy' objects}
        7    0.000    0.000    0.000    0.000 {method 'endswith' of 'str' objects}
        4    0.000    0.000    0.000    0.000 manager.py:91(add_ui_element)
        2    0.000    0.000    0.000    0.000 component.py:189(__init__)
        5    0.000    0.000    0.000    0.000 {method 'count' of 'str' objects}
       20    0.000    0.000    0.000    0.000 {built-in method builtins.abs}
        2    0.000    0.000    0.000    0.000 __init__.py:221(_releaseLock)
        3    0.000    0.000    0.000    0.000 esper.py:265(<listcomp>)
        3    0.000    0.000    0.000    0.000 chrono.py:138(get_time)
        1    0.000    0.000    0.000    0.000 warnings.py:419(__init__)
        5    0.000    0.000    0.000    0.000 __init__.py:122(unescape)
        2    0.000    0.000    0.000    0.000 _internal.py:305(data)
        1    0.000    0.000    0.000    0.000 entity_handler.py:26(__init__)
        2    0.000    0.000    0.000    0.000 library.py:169(get_skill_data)
        3    0.000    0.000    0.000    0.000 component.py:69(__init__)
        3    0.000    0.000    0.000    0.000 text_block.py:54(<listcomp>)
        3    0.000    0.000    0.000    0.000 __init__.py:102(_checkRange)
        3    0.000    0.000    0.000    0.000 _markupbase.py:36(reset)
        2    0.000    0.000    0.000    0.000 text_block.py:11(__init__)
        6    0.000    0.000    0.000    0.000 {method 'group' of 're.Match' objects}
        6    0.000    0.000    0.000    0.000 state.py:17(get_previous)
        2    0.000    0.000    0.000    0.000 component.py:37(__init__)
        3    0.000    0.000    0.000    0.000 component.py:138(__init__)
       12    0.000    0.000    0.000    0.000 {method 'items' of 'dict' objects}
        2    0.000    0.000    0.000    0.000 camera.py:186(set_tiles)
        5    0.000    0.000    0.000    0.000 manager.py:121(get_gui_manager)
        3    0.000    0.000    0.000    0.000 chrono.py:117(get_turn_holder)
        2    0.000    0.000    0.000    0.000 chrono.py:161(set_turn_holder)
        2    0.000    0.000    0.000    0.000 combat_stats.py:19(__init__)
        1    0.000    0.000    0.000    0.000 god_handler.py:23(__init__)
        4    0.000    0.000    0.000    0.000 {method 'join' of 'str' objects}
        4    0.000    0.000    0.000    0.000 ui_window.py:127(get_top_layer)
        8    0.000    0.000    0.000    0.000 {method 'setdefault' of 'dict' objects}
        3    0.000    0.000    0.000    0.000 {method 'sort' of 'list' objects}
        1    0.000    0.000    0.000    0.000 interaction_handler.py:22(__init__)
        2    0.000    0.000    0.000    0.000 __init__.py:1605(getEffectiveLevel)
        1    0.000    0.000    0.000    0.000 ui_handler.py:27(__init__)
        8    0.000    0.000    0.000    0.000 {method 'end' of 're.Match' objects}
        2    0.000    0.000    0.000    0.000 component.py:61(__init__)
        4    0.000    0.000    0.000    0.000 {method 'lstrip' of 'str' objects}
        2    0.000    0.000    0.000    0.000 component.py:78(__init__)
        5    0.000    0.000    0.000    0.000 {method 'get_ascent' of 'pygame.font.Font' objects}
        2    0.000    0.000    0.000    0.000 {method 'set_underline' of 'pygame.font.Font' objects}
        2    0.000    0.000    0.000    0.000 component.py:97(__init__)
        3    0.000    0.000    0.000    0.000 ui_appearance_theme.py:130(get_font_dictionary)
        1    0.000    0.000    0.000    0.000 chrono.py:108(add_time)
        2    0.000    0.000    0.000    0.000 component.py:106(__init__)
        1    0.000    0.000    0.000    0.000 library.py:231(get_god_data)
        2    0.000    0.000    0.000    0.000 chrono.py:131(get_time_in_round)
        2    0.000    0.000    0.000    0.000 component.py:115(__init__)
        1    0.000    0.000    0.000    0.000 component.py:181(__init__)
        1    0.000    0.000    0.000    0.000 chrono.py:175(set_turn_queue)
        1    0.000    0.000    0.000    0.000 ai.py:68(__init__)
        1    0.000    0.000    0.000    0.000 {method 'search' of 're.Pattern' objects}
        1    0.000    0.000    0.000    0.000 {method 'seek' of '_io.BufferedReader' objects}
        1    0.000    0.000    0.000    0.000 {method 'startswith' of 'bytes' objects}
        3    0.000    0.000    0.000    0.000 {method 'strip' of 'str' objects}
        1    0.000    0.000    0.000    0.000 codecs.py:260(__init__)
        1    0.000    0.000    0.000    0.000 component.py:123(__init__)
        1    0.000    0.000    0.000    0.000 camera.py:195(set_player_tile)
        1    0.000    0.000    0.000    0.000 {method 'decode' of 'bytes' objects}
        1    0.000    0.000    0.000    0.000 chrono.py:145(get_time_of_last_turn)
        1    0.000    0.000    0.000    0.000 parser.py:127(clear_cdata_mode)
        1    0.000    0.000    0.000    0.000 chrono.py:124(get_turn_queue)
        1    0.000    0.000    0.000    0.000 chrono.py:182(set_time_of_last_turn)
        2    0.000    0.000    0.000    0.000 {method 'upper' of 'str' objects}
        1    0.000    0.000    0.000    0.000 chrono.py:168(set_time_in_round)
        1    0.000    0.000    0.000    0.000 {method 'disable' of '_lsprof.Profiler' objects}


