Tue Mar 17 16:46:35 2020    logs/profiling/profile.dump

         2243004 function calls (2033728 primitive calls) in 9.081 seconds

   Ordered by: cumulative time

   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
        1    0.075    0.075    9.037    9.037 main.py:78(game_loop)
      468    5.237    0.011    5.237    0.011 {method 'tick' of 'Clock' objects}
      234    0.001    0.000    2.677    0.011 state.py:38(get_delta_time)
      234    0.001    0.000    2.562    0.011 state.py:63(update_clock)
      234    0.000    0.000    1.373    0.006 event_core.py:21(update)
       23    0.000    0.000    1.320    0.057 ui_handler.py:30(process_event)
        8    0.000    0.000    1.302    0.163 ui_handler.py:207(update_camera)
        8    0.000    0.000    1.280    0.160 manager.py:295(update_camera_grid)
        8    0.008    0.001    1.280    0.160 camera.py:106(update_grid)
     1205    0.015    0.000    1.250    0.001 ui_button.py:30(__init__)
     1205    0.066    0.000    1.176    0.001 ui_button.py:403(rebuild_from_changed_theme_data)
        9    0.000    0.000    1.143    0.127 ui_handler.py:48(process_entity_event)
      234    0.001    0.000    1.132    0.005 manager.py:54(update)
      234    0.062    0.000    1.131    0.005 ui_manager.py:122(update)
      234    0.003    0.000    1.080    0.005 manager.py:73(draw)
    35020    0.087    0.000    0.949    0.000 ui_appearance_theme.py:347(build_all_combined_ids)
244260/35020    0.810    0.000    0.855    0.000 ui_appearance_theme.py:322(get_next_id_node)
    76813    0.774    0.000    0.774    0.000 {method 'blit' of 'pygame.Surface' objects}
      234    0.037    0.000    0.718    0.003 sprite.py:453(update)
    18110    0.046    0.000    0.535    0.000 ui_appearance_theme.py:466(get_colour_or_gradient)
      241    0.221    0.001    0.466    0.002 camera.py:79(update_game_map)
      233    0.001    0.000    0.452    0.002 camera.py:72(update)
      234    0.002    0.000    0.403    0.002 ui_manager.py:173(draw_ui)
      234    0.063    0.000    0.401    0.002 sprite.py:753(draw)
      238    0.317    0.001    0.317    0.001 {built-in method pygame.transform.scale}
    10880    0.021    0.000    0.313    0.000 ui_appearance_theme.py:428(get_misc_data)
    36764    0.149    0.000    0.267    0.000 ui_element.py:121(check_hover)
    36115    0.071    0.000    0.201    0.000 ui_button.py:197(update)
       12    0.000    0.000    0.173    0.014 ui_handler.py:72(process_game_event)
        1    0.000    0.000    0.168    0.168 ui_handler.py:111(init_game_ui)
     1205    0.007    0.000    0.146    0.000 ui_button.py:97(set_any_images_from_theme)
     4820    0.009    0.000    0.139    0.000 ui_appearance_theme.py:366(get_image)
    36115    0.028    0.000    0.116    0.000 drawable_shape.py:36(update)
      234    0.114    0.000    0.114    0.000 {built-in method pygame.event.get}
     6030    0.038    0.000    0.110    0.000 rect_drawable_shape.py:118(redraw_state)
    36115    0.043    0.000    0.082    0.000 ui_button.py:138(hover_point)
      234    0.081    0.000    0.081    0.000 {built-in method pygame.display.flip}
     2513    0.075    0.000    0.078    0.000 sprite.py:913(get_sprites_from_layer)
     1205    0.009    0.000    0.069    0.000 ui_button.py:537(rebuild_shape)
     1210    0.004    0.000    0.060    0.000 rect_drawable_shape.py:22(__init__)
     1220    0.012    0.000    0.055    0.000 ui_element.py:23(__init__)
     1210    0.017    0.000    0.053    0.000 rect_drawable_shape.py:32(full_rebuild_on_size_change)
    36157    0.051    0.000    0.051    0.000 camera.py:234(world_to_screen_position)
     1205    0.005    0.000    0.047    0.000 ui_appearance_theme.py:405(get_font)
       21    0.000    0.000    0.045    0.002 entity_handler.py:28(process_event)
        1    0.000    0.000    0.044    0.044 main.py:183(initialise_game)
    10507    0.013    0.000    0.043    0.000 _internal.py:24(wrapper)
        7    0.000    0.000    0.040    0.006 entity_handler.py:53(_process_move)
        2    0.000    0.000    0.040    0.020 entity.py:225(create_actor)
        5    0.010    0.002    0.039    0.008 world.py:445(update_tile_visibility)
    36115    0.034    0.000    0.039    0.000 rect_drawable_shape.py:84(collide_point)
   512781    0.034    0.000    0.034    0.000 {method 'append' of 'list' objects}
      501    0.034    0.000    0.034    0.000 {method 'fill' of 'pygame.Surface' objects}
        2    0.008    0.004    0.033    0.016 world.py:26(create_fov_map)
     1210    0.006    0.000    0.032    0.000 drawable_shape.py:45(redraw_all_states)
     6030    0.032    0.000    0.032    0.000 surface_cache.py:119(build_cache_id)
    74694    0.024    0.000    0.030    0.000 sprite.py:208(alive)
     6048    0.028    0.000    0.028    0.000 {method 'copy' of 'pygame.Surface' objects}
     1220    0.003    0.000    0.026    0.000 ui_container.py:42(add_element)
        8    0.005    0.001    0.025    0.003 ui_container.py:116(clear)
   438036    0.022    0.000    0.022    0.000 {built-in method builtins.len}
      183    0.001    0.000    0.022    0.000 screen_message.py:34(update)
    10508    0.021    0.000    0.021    0.000 {built-in method _warnings.warn}
     2274    0.021    0.000    0.021    0.000 ui_container.py:62(recalculate_container_layer_thickness)
     1050    0.001    0.000    0.020    0.000 ui_button.py:130(kill)
       34    0.000    0.000    0.019    0.001 manager.py:60(process_ui_events)
       34    0.007    0.000    0.019    0.001 ui_manager.py:86(process_events)
     1054    0.002    0.000    0.018    0.000 ui_element.py:114(kill)
      123    0.000    0.000    0.018    0.000 ui_text_box.py:347(redraw_from_chunks)
     1220    0.002    0.000    0.016    0.000 sprite.py:121(__init__)
        8    0.000    0.000    0.016    0.002 manager.py:286(update_camera_game_map)
      416    0.003    0.000    0.016    0.000 ui_text_box.py:205(update)
     4232    0.009    0.000    0.015    0.000 world.py:55(get_tile)
    36115    0.014    0.000    0.014    0.000 ui_button.py:154(can_hover)
     1220    0.005    0.000    0.014    0.000 sprite.py:126(add)
      123    0.002    0.000    0.012    0.000 ui_text_box.py:327(redraw_from_text_block)
      102    0.001    0.000    0.012    0.000 __init__.py:1496(_log)
       88    0.000    0.000    0.011    0.000 __init__.py:1996(debug)
       88    0.000    0.000    0.011    0.000 __init__.py:1361(debug)
     1220    0.002    0.000    0.011    0.000 ui_element.py:104(change_layer)
     1054    0.002    0.000    0.011    0.000 ui_container.py:52(remove_element)
        5    0.000    0.000    0.010    0.002 ui_text_box.py:50(__init__)
        5    0.000    0.000    0.009    0.002 ui_text_box.py:492(rebuild_from_changed_theme_data)
     6030    0.008    0.000    0.009    0.000 drawable_shape.py:122(rebuild_images_and_text)
   139640    0.009    0.000    0.009    0.000 {method 'reverse' of 'list' objects}
     1228    0.007    0.000    0.009    0.000 sprite.py:822(change_layer)
     1220    0.008    0.000    0.008    0.000 sprite.py:646(add_internal)
        5    0.000    0.000    0.008    0.002 ui_text_box.py:110(rebuild)
     1232    0.003    0.000    0.007    0.000 ui_font_dictionary.py:89(find_font)
        5    0.000    0.000    0.007    0.001 ui_text_box.py:310(parse_html_into_style_data)
    38579    0.007    0.000    0.007    0.000 ui_manager.py:167(get_mouse_position)
       62    0.000    0.000    0.007    0.000 ui_text_box.py:462(set_active_effect)
      102    0.000    0.000    0.006    0.000 __init__.py:1521(handle)
        2    0.000    0.000    0.006    0.003 entity.py:328(build_characteristic_sprites)
     7500    0.003    0.000    0.006    0.000 libtcodpy.py:3300(map_is_in_fov)
       40    0.000    0.000    0.006    0.000 utility.py:13(get_image)
        8    0.000    0.000    0.006    0.001 manager.py:275(update_cameras_tiles)
        8    0.002    0.000    0.006    0.001 camera.py:168(update_camera_tiles)
      102    0.000    0.000    0.006    0.000 __init__.py:1575(callHandlers)
        5    0.000    0.000    0.006    0.001 text_block.py:16(__init__)
        5    0.000    0.000    0.006    0.001 text_block.py:40(redraw)
      102    0.000    0.000    0.006    0.000 __init__.py:892(handle)
       12    0.000    0.000    0.005    0.000 utility.py:39(get_images)
     1054    0.002    0.000    0.005    0.000 sprite.py:183(kill)
      123    0.002    0.000    0.005    0.000 text_block.py:265(redraw_from_chunks)
     1399    0.004    0.000    0.005    0.000 ui_container.py:124(check_hover)
    38163    0.005    0.000    0.005    0.000 {method 'collidepoint' of 'pygame.Rect' objects}
    74694    0.005    0.000    0.005    0.000 {built-in method _operator.truth}
       41    0.005    0.000    0.005    0.000 {built-in method pygame.imageext.load_extended}
      102    0.000    0.000    0.005    0.000 __init__.py:1123(emit)
    35711    0.005    0.000    0.005    0.000 {method 'union' of 'pygame.Rect' objects}
     4241    0.004    0.000    0.005    0.000 world.py:347(_is_tile_in_bounds)
      102    0.000    0.000    0.005    0.000 __init__.py:1022(emit)
        1    0.000    0.000    0.005    0.005 manager.py:223(create_screen_message)
        1    0.000    0.000    0.005    0.005 screen_message.py:16(__init__)
        3    0.000    0.000    0.004    0.001 message_log.py:49(add_message)
      234    0.001    0.000    0.004    0.000 processors.py:16(process_all)
      102    0.000    0.000    0.004    0.000 __init__.py:1481(makeRecord)
    38320    0.004    0.000    0.004    0.000 {method 'colliderect' of 'pygame.Rect' objects}
       12    0.000    0.000    0.004    0.000 game_handler.py:26(process_event)
        1    0.000    0.000    0.004    0.004 manager.py:182(init_skill_bar)
        1    0.000    0.000    0.004    0.004 skill_bar.py:15(__init__)
      102    0.001    0.000    0.004    0.000 __init__.py:293(__init__)
     6030    0.004    0.000    0.004    0.000 surface_cache.py:109(find_surface_in_cache)
     1054    0.002    0.000    0.004    0.000 sprite.py:728(remove_internal)
     1210    0.003    0.000    0.003    0.000 drawable_shape.py:11(__init__)
        1    0.000    0.000    0.003    0.003 world.py:19(create_game_map)
        1    0.002    0.002    0.003    0.003 game_map.py:12(__init__)
      234    0.003    0.000    0.003    0.000 processors.py:23(_process_aesthetic_update)
        1    0.000    0.000    0.003    0.003 manager.py:156(init_message_log)
     3000    0.001    0.000    0.003    0.000 libtcodpy.py:3254(map_set_properties)
        1    0.000    0.000    0.003    0.003 message_log.py:18(__init__)
     1216    0.002    0.000    0.003    0.000 ui_element.py:68(create_valid_ids)
     7500    0.003    0.000    0.003    0.000 {built-in method tcod._libtcod.TCOD_map_is_in_fov}
     3565    0.003    0.000    0.003    0.000 ui_button.py:257(process_event)
      268    0.002    0.000    0.003    0.000 sprite.py:814(layers)
        2    0.000    0.000    0.002    0.001 ui_handler.py:155(process_ui_event)
        2    0.000    0.000    0.002    0.001 ui_handler.py:238(process_message)
        2    0.000    0.000    0.002    0.001 manager.py:444(add_to_message_log)
      102    0.000    0.000    0.002    0.000 __init__.py:869(format)
      234    0.001    0.000    0.002    0.000 ui_appearance_theme.py:158(update_shape_cache)
     1166    0.002    0.000    0.002    0.000 ui_window.py:97(update)
      102    0.001    0.000    0.002    0.000 __init__.py:606(format)
        1    0.002    0.002    0.002    0.002 ui_font_dictionary.py:155(preload_font)
        4    0.000    0.000    0.002    0.000 interaction_handler.py:25(process_event)
     2300    0.002    0.000    0.002    0.000 {method 'remove' of 'list' objects}
      232    0.002    0.000    0.002    0.000 ui_manager.py:104(<listcomp>)
     1233    0.002    0.000    0.002    0.000 ui_font_dictionary.py:133(create_font_id)
      234    0.001    0.000    0.002    0.000 ui_manager.py:158(update_mouse_position)
       14    0.000    0.000    0.002    0.000 __init__.py:1986(info)
      234    0.000    0.000    0.002    0.000 surface_cache.py:24(update)
       14    0.000    0.000    0.002    0.000 __init__.py:1373(info)
     3000    0.002    0.000    0.002    0.000 {built-in method tcod._libtcod.TCOD_map_set_properties}
      102    0.000    0.000    0.002    0.000 __init__.py:1011(flush)
      468    0.002    0.000    0.002    0.000 sprite.py:745(sprites)
     4840    0.002    0.000    0.002    0.000 {built-in method math.floor}
      104    0.000    0.000    0.001    0.000 ntpath.py:212(basename)
        7    0.000    0.000    0.001    0.000 styled_chunk.py:8(__init__)
     1210    0.001    0.000    0.001    0.000 drawable_shape.py:50(compute_aligned_text_rect)
        9    0.000    0.000    0.001    0.000 god_handler.py:26(process_event)
        2    0.000    0.000    0.001    0.001 interaction_handler.py:86(_process_entity_collision)
        2    0.000    0.000    0.001    0.001 entity_handler.py:131(_process_skill)
        2    0.000    0.000    0.001    0.001 interaction_handler.py:121(_apply_effects_to_tiles)
       32    0.001    0.000    0.001    0.000 {method 'render' of 'pygame.font.Font' objects}
      104    0.001    0.000    0.001    0.000 ntpath.py:178(split)
     8482    0.001    0.000    0.001    0.000 world.py:48(get_game_map)
      102    0.001    0.000    0.001    0.000 __init__.py:1451(findCaller)
        8    0.000    0.000    0.001    0.000 game_handler.py:42(process_change_game_state)
      233    0.001    0.000    0.001    0.000 skill_bar.py:45(update)
      102    0.000    0.000    0.001    0.000 __init__.py:539(formatTime)
        3    0.000    0.000    0.001    0.000 game_handler.py:81(process_end_turn)
      537    0.001    0.000    0.001    0.000 {built-in method builtins.sorted}
        3    0.000    0.000    0.001    0.000 chrono.py:50(next_turn)
      102    0.001    0.000    0.001    0.000 {method 'flush' of '_io.TextIOWrapper' objects}
       14    0.001    0.000    0.001    0.000 {method 'metrics' of 'pygame.font.Font' objects}
        7    0.000    0.000    0.001    0.000 state.py:71(set_new)
     1249    0.001    0.000    0.001    0.000 ui_window_stack.py:73(get_root_window)
        9    0.000    0.000    0.001    0.000 surface_cache.py:29(add_surface_to_long_term_cache)
      233    0.000    0.000    0.001    0.000 message_log.py:36(update)
      649    0.001    0.000    0.001    0.000 ui_element.py:186(hover_point)
     2398    0.001    0.000    0.001    0.000 ui_window.py:107(get_container)
     1210    0.001    0.000    0.001    0.000 drawable_shape.py:46(<listcomp>)
     6030    0.001    0.000    0.001    0.000 {method 'popleft' of 'collections.deque' objects}
     1500    0.001    0.000    0.001    0.000 tile.py:20(__init__)
        1    0.000    0.000    0.001    0.001 manager.py:195(init_camera)
        1    0.000    0.000    0.001    0.001 camera.py:24(__init__)
       34    0.000    0.000    0.001    0.000 processors.py:57(process_intent)
     1221    0.001    0.000    0.001    0.000 drawable_shape.py:86(get_surface)
     2468    0.001    0.000    0.001    0.000 {built-in method builtins.min}
        5    0.000    0.000    0.001    0.000 parser.py:104(feed)
     3958    0.001    0.000    0.001    0.000 {built-in method builtins.hasattr}
        5    0.000    0.000    0.001    0.000 parser.py:134(goahead)
     2448    0.001    0.000    0.001    0.000 {method 'insert' of 'list' objects}
      233    0.000    0.000    0.001    0.000 entity_info.py:45(update)
      102    0.000    0.000    0.001    0.000 ntpath.py:201(splitext)
     2358    0.001    0.000    0.001    0.000 {built-in method builtins.max}
      103    0.000    0.000    0.001    0.000 {method 'write' of '_io.TextIOWrapper' objects}
     5097    0.001    0.000    0.001    0.000 {built-in method builtins.isinstance}
      102    0.001    0.000    0.001    0.000 {built-in method time.strftime}
      234    0.001    0.000    0.001    0.000 {built-in method pygame.mouse.get_pos}
        2    0.000    0.000    0.001    0.000 pydevd_modify_bytecode.py:213(insert_code)
      481    0.001    0.000    0.001    0.000 state.py:45(get_current)
        6    0.000    0.000    0.001    0.000 dataclasses.py:1023(asdict)
        4    0.000    0.000    0.001    0.000 ui_window.py:18(__init__)
        1    0.000    0.000    0.001    0.001 pydevd_modify_bytecode.py:233(_insert_code)
       23    0.000    0.000    0.001    0.000 processors.py:138(_process_player_turn_intents)
     42/6    0.000    0.000    0.001    0.000 dataclasses.py:1047(_asdict_inner)
        1    0.000    0.000    0.001    0.001 warnings.py:96(_showwarnmsg)
        1    0.000    0.000    0.001    0.001 warnings.py:20(_showwarnmsg_impl)
      208    0.000    0.000    0.000    0.000 ntpath.py:44(normcase)
        2    0.000    0.000    0.000    0.000 skill.py:201(process_effect)
     2449    0.000    0.000    0.000    0.000 ui_manager.py:44(get_sprite_group)
        1    0.000    0.000    0.000    0.000 pydevd_modify_bytecode.py:128(_update_label_offsets)
     1220    0.000    0.000    0.000    0.000 sprite.py:162(add_internal)
        2    0.000    0.000    0.000    0.000 entity.py:294(create_projectile)
        1    0.000    0.000    0.000    0.000 entity.py:194(create_god)
      102    0.000    0.000    0.000    0.000 genericpath.py:117(_splitext)
        5    0.000    0.000    0.000    0.000 html_parser.py:207(__init__)
       35    0.000    0.000    0.000    0.000 {method 'convert_alpha' of 'pygame.Surface' objects}
      108    0.000    0.000    0.000    0.000 ntpath.py:122(splitdrive)
     2412    0.000    0.000    0.000    0.000 {method 'copy' of 'list' objects}
       34    0.000    0.000    0.000    0.000 action.py:12(convert_to_intent)
        5    0.000    0.000    0.000    0.000 html_parser.py:60(__init__)
        5    0.000    0.000    0.000    0.000 {built-in method nt.stat}
        1    0.000    0.000    0.000    0.000 warnings.py:117(_formatwarnmsg)
        1    0.000    0.000    0.000    0.000 warnings.py:35(_formatwarnmsg_impl)
        1    0.000    0.000    0.000    0.000 linecache.py:15(getline)
        1    0.000    0.000    0.000    0.000 linecache.py:37(getlines)
     1228    0.000    0.000    0.000    0.000 {method 'pop' of 'dict' objects}
        1    0.000    0.000    0.000    0.000 linecache.py:82(updatecache)
        4    0.000    0.000    0.000    0.000 ui_appearance_theme.py:138(check_need_to_reload)
        7    0.000    0.000    0.000    0.000 parser.py:301(parse_starttag)
       44    0.000    0.000    0.000    0.000 {built-in method pygame.transform.smoothscale}
     1632    0.000    0.000    0.000    0.000 sprite.py:168(update)
        1    0.000    0.000    0.000    0.000 chrono.py:23(rebuild_turn_queue)
       26    0.000    0.000    0.000    0.000 {method 'size' of 'pygame.font.Font' objects}
      102    0.000    0.000    0.000    0.000 __init__.py:590(formatMessage)
       23    0.000    0.000    0.000    0.000 utility.py:188(value_to_member)
        1    0.000    0.000    0.000    0.000 manager.py:169(init_entity_info)
      234    0.000    0.000    0.000    0.000 {built-in method builtins.any}
       20    0.000    0.000    0.000    0.000 entity.py:123(get_primary_stat)
        5    0.000    0.000    0.000    0.000 entity.py:166(create)
        2    0.000    0.000    0.000    0.000 skill.py:92(pay_resource_cost)
        1    0.000    0.000    0.000    0.000 entity_info.py:19(__init__)
        2    0.000    0.000    0.000    0.000 skill.py:73(can_afford_cost)
     1219    0.000    0.000    0.000    0.000 ui_manager.py:51(get_window_stack)
     1220    0.000    0.000    0.000    0.000 {method '__contains__' of 'dict' objects}
        5    0.000    0.000    0.000    0.000 ui_container.py:19(__init__)
       67    0.000    0.000    0.000    0.000 entity.py:86(get_entitys_component)
      102    0.000    0.000    0.000    0.000 {built-in method time.gmtime}
      102    0.000    0.000    0.000    0.000 __init__.py:584(usesTime)
       14    0.000    0.000    0.000    0.000 world.py:260(tile_has_tag)
     1166    0.000    0.000    0.000    0.000 ui_window.py:116(check_hover)
       62    0.000    0.000    0.000    0.000 text_effects.py:81(__init__)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:555(get_abs_path_real_path_and_base_from_file)
     1220    0.000    0.000    0.000    0.000 ui_manager.py:37(get_theme)
      102    0.000    0.000    0.000    0.000 cp1252.py:18(encode)
       43    0.000    0.000    0.000    0.000 entity.py:321(add_component)
       13    0.000    0.000    0.000    0.000 html_parser.py:118(add_text)
        2    0.000    0.000    0.000    0.000 combat_stats.py:67(max_health)
     1091    0.000    0.000    0.000    0.000 {method 'pop' of 'list' objects}
     1209    0.000    0.000    0.000    0.000 {method 'copy' of 'pygame.Rect' objects}
      204    0.000    0.000    0.000    0.000 __init__.py:849(acquire)
        8    0.000    0.000    0.000    0.000 esper.py:274(get_components)
        5    0.000    0.000    0.000    0.000 world.py:438(recompute_fov)
      102    0.000    0.000    0.000    0.000 __init__.py:432(format)
       25    0.000    0.000    0.000    0.000 utility.py:94(get_class_members)
       61    0.000    0.000    0.000    0.000 text_effects.py:88(update)
     1205    0.000    0.000    0.000    0.000 drawable_shape.py:33(clean_up_temp_shapes)
       13    0.000    0.000    0.000    0.000 html_parser.py:123(add_indexed_style)
       43    0.000    0.000    0.000    0.000 esper.py:196(add_component)
      120    0.000    0.000    0.000    0.000 pydevd_modify_bytecode.py:103(_unpack_opargs)
        9    0.000    0.000    0.000    0.000 surface_cache.py:21(add_surface_to_cache)
        8    0.000    0.000    0.000    0.000 esper.py:276(<listcomp>)
        7    0.000    0.000    0.000    0.000 html_parser.py:213(handle_starttag)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:228(_NormPaths)
        2    0.000    0.000    0.000    0.000 combat_stats.py:92(max_stamina)
      649    0.000    0.000    0.000    0.000 ui_element.py:204(can_hover)
       19    0.000    0.000    0.000    0.000 entity.py:96(get_name)
        4    0.000    0.000    0.000    0.000 pydevd_file_utils.py:245(_NormPath)
       14    0.000    0.000    0.000    0.000 surface_cache.py:80(split_rect)
       12    0.000    0.000    0.000    0.000 utility.py:51(flatten_images)
     1054    0.000    0.000    0.000    0.000 {method 'clear' of 'dict' objects}
       30    0.000    0.000    0.000    0.000 utility.py:107(lerp)
      204    0.000    0.000    0.000    0.000 __init__.py:856(release)
      204    0.000    0.000    0.000    0.000 __init__.py:747(filter)
        7    0.000    0.000    0.000    0.000 html_parser.py:283(handle_data)
        2    0.000    0.000    0.000    0.000 god_handler.py:74(process_interventions)
      102    0.000    0.000    0.000    0.000 __init__.py:429(usesTime)
        5    0.000    0.000    0.000    0.000 ui_manager.py:59(get_shadow)
       39    0.000    0.000    0.000    0.000 entity.py:37(get_player)
        1    0.000    0.000    0.000    0.000 tokenize.py:443(open)
      102    0.000    0.000    0.000    0.000 __init__.py:154(<lambda>)
       22    0.000    0.000    0.000    0.000 esper.py:254(_get_components)
        4    0.000    0.000    0.000    0.000 ntpath.py:523(abspath)
        2    0.000    0.000    0.000    0.000 libtcodpy.py:3228(map_new)
        1    0.000    0.000    0.000    0.000 __init__.py:316(namedtuple)
        9    0.000    0.000    0.000    0.000 esper.py:270(get_component)
        2    0.000    0.000    0.000    0.000 entity.py:416(consider_intervening)
      309    0.000    0.000    0.000    0.000 {method 'values' of 'dict' objects}
        7    0.000    0.000    0.000    0.000 world.py:359(_is_tile_blocking_movement)
       19    0.000    0.000    0.000    0.000 entity.py:109(get_identity)
        1    0.000    0.000    0.000    0.000 {method 'readlines' of '_io._IOBase' objects}
      102    0.000    0.000    0.000    0.000 {built-in method _codecs.charmap_encode}
       36    0.000    0.000    0.000    0.000 copy.py:132(deepcopy)
        2    0.000    0.000    0.000    0.000 map.py:66(__init__)
        5    0.000    0.000    0.000    0.000 ui_shadow.py:178(find_closest_shadow_scale_to_size)
        7    0.000    0.000    0.000    0.000 manager.py:345(should_camera_move)
      102    0.000    0.000    0.000    0.000 __init__.py:117(getLevelName)
        4    0.000    0.000    0.000    0.000 ui_text_box.py:102(kill)
       27    0.000    0.000    0.000    0.000 event_core.py:38(publish)
       34    0.000    0.000    0.000    0.000 action.py:34(_check_directions)
        4    0.000    0.000    0.000    0.000 ntpath.py:450(normpath)
        4    0.000    0.000    0.000    0.000 ui_window_stack.py:23(add_new_window)
      306    0.000    0.000    0.000    0.000 {method 'rfind' of 'str' objects}
       15    0.000    0.000    0.000    0.000 __init__.py:186(easeOutCubic)
      128    0.000    0.000    0.000    0.000 {method 'set_alpha' of 'pygame.Surface' objects}
        2    0.000    0.000    0.000    0.000 skill.py:227(_process_trigger_skill_effect)
      102    0.000    0.000    0.000    0.000 threading.py:1206(current_thread)
       23    0.000    0.000    0.000    0.000 processors.py:71(_get_pressed_direction)
        9    0.000    0.000    0.000    0.000 esper.py:272(<listcomp>)
      126    0.000    0.000    0.000    0.000 ui_window.py:55(process_event)
        4    0.000    0.000    0.000    0.000 combat_stats.py:22(vigour)
      321    0.000    0.000    0.000    0.000 {method 'replace' of 'str' objects}
      102    0.000    0.000    0.000    0.000 __init__.py:1619(isEnabledFor)
      337    0.000    0.000    0.000    0.000 {method 'get' of 'dict' objects}
      102    0.000    0.000    0.000    0.000 __init__.py:371(getMessage)
      526    0.000    0.000    0.000    0.000 {built-in method nt.fspath}
        5    0.000    0.000    0.000    0.000 ui_appearance_theme.py:386(get_font_info)
        2    0.000    0.000    0.000    0.000 pydevd_frame_tracing.py:96(create_pydev_trace_code_wrapper)
        4    0.000    0.000    0.000    0.000 ui_window.py:135(change_window_layer)
       37    0.000    0.000    0.000    0.000 ui_text_box.py:379(process_event)
        7    0.000    0.000    0.000    0.000 world.py:395(_tile_has_other_entity)
        5    0.000    0.000    0.000    0.000 libtcodpy.py:3283(map_compute_fov)
        2    0.000    0.000    0.000    0.000 {built-in method builtins.compile}
        2    0.000    0.000    0.000    0.000 map.py:74(__as_cdata)
      104    0.000    0.000    0.000    0.000 ntpath.py:34(_get_bothseps)
      206    0.000    0.000    0.000    0.000 {method 'acquire' of '_thread.RLock' objects}
        6    0.000    0.000    0.000    0.000 dataclasses.py:994(fields)
       34    0.000    0.000    0.000    0.000 processors.py:118(_process_stateless_intents)
       45    0.000    0.000    0.000    0.000 ui_button.py:170(while_hovering)
      292    0.000    0.000    0.000    0.000 {method 'lower' of 'str' objects}
       32    0.000    0.000    0.000    0.000 utility.py:121(clamp)
        4    0.000    0.000    0.000    0.000 combat_stats.py:31(clout)
        4    0.000    0.000    0.000    0.000 combat_stats.py:40(skullduggery)
      118    0.000    0.000    0.000    0.000 {method 'find' of 'str' objects}
      102    0.000    0.000    0.000    0.000 threading.py:1052(name)
        4    0.000    0.000    0.000    0.000 combat_stats.py:49(bustle)
        4    0.000    0.000    0.000    0.000 combat_stats.py:58(exactitude)
       17    0.000    0.000    0.000    0.000 html_parser.py:94(push_style)
        5    0.000    0.000    0.000    0.000 map.py:100(compute_fov)
        1    0.000    0.000    0.000    0.000 {built-in method io.open}
       43    0.000    0.000    0.000    0.000 esper.py:51(clear_cache)
       51    0.000    0.000    0.000    0.000 dis.py:436(findlinestarts)
       81    0.000    0.000    0.000    0.000 esper.py:176(has_component)
      172    0.000    0.000    0.000    0.000 ui_element.py:210(process_event)
       27    0.000    0.000    0.000    0.000 event_core.py:12(notify)
      258    0.000    0.000    0.000    0.000 {built-in method builtins.getattr}
        7    0.000    0.000    0.000    0.000 manager.py:398(world_to_screen_position)
       35    0.000    0.000    0.000    0.000 {method 'match' of 're.Pattern' objects}
        1    0.000    0.000    0.000    0.000 main.py:210(initialise_event_handlers)
       23    0.000    0.000    0.000    0.000 processors.py:98(_get_pressed_skills_number)
        9    0.000    0.000    0.000    0.000 {method 'subsurface' of 'pygame.Surface' objects}
        1    0.000    0.000    0.000    0.000 ui_image.py:20(__init__)
        7    0.000    0.000    0.000    0.000 event.py:53(__init__)
       48    0.000    0.000    0.000    0.000 dataclasses.py:1012(_is_dataclass_instance)
      346    0.000    0.000    0.000    0.000 {method 'contains' of 'pygame.Rect' objects}
        1    0.000    0.000    0.000    0.000 {built-in method builtins.exec}
      103    0.000    0.000    0.000    0.000 {built-in method sys._getframe}
        5    0.000    0.000    0.000    0.000 {built-in method tcod._libtcod.TCOD_map_compute_fov}
       62    0.000    0.000    0.000    0.000 text_effects.py:2(__init__)
       28    0.000    0.000    0.000    0.000 esper.py:243(_get_component)
        7    0.000    0.000    0.000    0.000 parser.py:352(check_for_whole_start_tag)
      102    0.000    0.000    0.000    0.000 {built-in method time.time}
        5    0.000    0.000    0.000    0.000 parser.py:87(__init__)
        1    0.000    0.000    0.000    0.000 tokenize.py:350(detect_encoding)
       14    0.000    0.000    0.000    0.000 manager.py:236(is_target_pos_in_camera_edge)
      102    0.000    0.000    0.000    0.000 {built-in method nt.getpid}
        1    0.000    0.000    0.000    0.000 ui_shadow.py:99(create_new_rectangle_shadow)
      123    0.000    0.000    0.000    0.000 text_effects.py:107(get_final_alpha)
      104    0.000    0.000    0.000    0.000 {method 'rstrip' of 'str' objects}
      120    0.000    0.000    0.000    0.000 esper.py:278(try_component)
       27    0.000    0.000    0.000    0.000 event_core.py:62(__init__)
      204    0.000    0.000    0.000    0.000 {built-in method _thread.get_ident}
        2    0.000    0.000    0.000    0.000 world.py:77(get_direction)
        2    0.000    0.000    0.000    0.000 random.py:344(choices)
        4    0.000    0.000    0.000    0.000 ui_container.py:75(change_container_layer)
       62    0.000    0.000    0.000    0.000 manager.py:128(get_ui_element)
        7    0.000    0.000    0.000    0.000 manager.py:305(set_player_tile)
       16    0.000    0.000    0.000    0.000 _markupbase.py:48(updatepos)
       61    0.000    0.000    0.000    0.000 text_effects.py:100(should_redraw_from_chunks)
       67    0.000    0.000    0.000    0.000 esper.py:146(component_for_entity)
      206    0.000    0.000    0.000    0.000 {method 'release' of '_thread.RLock' objects}
        8    0.000    0.000    0.000    0.000 abc.py:137(__instancecheck__)
        8    0.000    0.000    0.000    0.000 event.py:87(__init__)
        3    0.000    0.000    0.000    0.000 entity_handler.py:229(_process_end_turn)
        3    0.000    0.000    0.000    0.000 pydevd_modify_bytecode.py:12(_add_attr_values_from_insert_to_original)
       27    0.000    0.000    0.000    0.000 action.py:60(_check_actions)
        5    0.000    0.000    0.000    0.000 parser.py:96(reset)
        9    0.000    0.000    0.000    0.000 surface_cache.py:68(<listcomp>)
        1    0.000    0.000    0.000    0.000 pydevd_modify_bytecode.py:48(_modify_new_lines)
        1    0.000    0.000    0.000    0.000 parser.py:386(parse_endtag)
        2    0.000    0.000    0.000    0.000 entity.py:67(get_entities_and_components_in_area)
       61    0.000    0.000    0.000    0.000 text_effects.py:5(should_full_redraw)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:550(is_real_file)
        5    0.000    0.000    0.000    0.000 ui_button.py:187(on_unhovered)
        8    0.000    0.000    0.000    0.000 {built-in method _abc._abc_instancecheck}
       26    0.000    0.000    0.000    0.000 action.py:83(_check_dev_actions)
        8    0.000    0.000    0.000    0.000 event_core.py:50(subscribe)
        1    0.000    0.000    0.000    0.000 tokenize.py:374(read_or_stop)
       18    0.000    0.000    0.000    0.000 html_parser.py:8(__init__)
       13    0.000    0.000    0.000    0.000 html_parser.py:27(__init__)
        2    0.000    0.000    0.000    0.000 world.py:106(get_tiles)
        3    0.000    0.000    0.000    0.000 entity.py:366(spend_time)
        2    0.000    0.000    0.000    0.000 {built-in method numpy.zeros}
        2    0.000    0.000    0.000    0.000 {method 'new' of 'CompiledFFI' objects}
        6    0.000    0.000    0.000    0.000 ui_button.py:162(on_hovered)
        1    0.000    0.000    0.000    0.000 {method 'readline' of '_io.BufferedReader' objects}
       42    0.000    0.000    0.000    0.000 dataclasses.py:1009(<genexpr>)
        2    0.000    0.000    0.000    0.000 pydevd_file_utils.py:542(_is_int)
       86    0.000    0.000    0.000    0.000 {method 'cache_clear' of 'functools._lru_cache_wrapper' objects}
        3    0.000    0.000    0.000    0.000 codecs.py:319(decode)
      144    0.000    0.000    0.000    0.000 {method 'keys' of 'dict' objects}
       15    0.000    0.000    0.000    0.000 __init__.py:102(_checkRange)
        4    0.000    0.000    0.000    0.000 pydevd_file_utils.py:171(normcase)
       14    0.000    0.000    0.000    0.000 esper.py:266(<listcomp>)
        8    0.000    0.000    0.000    0.000 esper.py:265(<listcomp>)
       25    0.000    0.000    0.000    0.000 {method 'keys' of 'mappingproxy' objects}
        9    0.000    0.000    0.000    0.000 surface_cache.py:62(<listcomp>)
        1    0.000    0.000    0.000    0.000 manager.py:264(move_camera)
        2    0.000    0.000    0.000    0.000 event.py:30(__init__)
        9    0.000    0.000    0.000    0.000 {method 'get_size' of 'pygame.Surface' objects}
        8    0.000    0.000    0.000    0.000 event_core.py:15(subscribe)
        2    0.000    0.000    0.000    0.000 random.py:365(<listcomp>)
        2    0.000    0.000    0.000    0.000 event.py:126(__init__)
        7    0.000    0.000    0.000    0.000 styled_chunk.py:73(unset_underline_style)
        2    0.000    0.000    0.000    0.000 _internal.py:251(__init__)
        8    0.000    0.000    0.000    0.000 camera.py:186(set_tiles)
        3    0.000    0.000    0.000    0.000 event.py:69(__init__)
        4    0.000    0.000    0.000    0.000 abc.py:141(__subclasscheck__)
       36    0.000    0.000    0.000    0.000 {method 'update' of 'dict' objects}
       22    0.000    0.000    0.000    0.000 library.py:155(get_homeland_data)
       13    0.000    0.000    0.000    0.000 chrono.py:114(get_turn_holder)
        3    0.000    0.000    0.000    0.000 {built-in method _codecs.utf_8_decode}
        7    0.000    0.000    0.000    0.000 camera.py:195(set_player_tile)
        5    0.000    0.000    0.000    0.000 {built-in method math.sin}
       15    0.000    0.000    0.000    0.000 {method 'count' of 'str' objects}
        2    0.000    0.000    0.000    0.000 {method 'cast' of 'CompiledFFI' objects}
        5    0.000    0.000    0.000    0.000 esper.py:100(create_entity)
        2    0.000    0.000    0.000    0.000 entity.py:116(get_combat_stats)
        1    0.000    0.000    0.000    0.000 html_parser.py:272(handle_endtag)
        2    0.000    0.000    0.000    0.000 event.py:174(__init__)
       10    0.000    0.000    0.000    0.000 library.py:169(get_skill_data)
        5    0.000    0.000    0.000    0.000 text_block.py:288(add_chunks_to_hover_group)
        4    0.000    0.000    0.000    0.000 {built-in method nt._getfullpathname}
       22    0.000    0.000    0.000    0.000 {method 'startswith' of 'str' objects}
       22    0.000    0.000    0.000    0.000 library.py:139(get_people_data)
       22    0.000    0.000    0.000    0.000 library.py:123(get_savvy_data)
       36    0.000    0.000    0.000    0.000 copy.py:190(_deepcopy_atomic)
        2    0.000    0.000    0.000    0.000 event.py:108(__init__)
        3    0.000    0.000    0.000    0.000 component.py:46(__init__)
        1    0.000    0.000    0.000    0.000 camera.py:224(move_camera)
        1    0.000    0.000    0.000    0.000 main.py:160(disable_profiling)
        1    0.000    0.000    0.000    0.000 event.py:79(__init__)
        5    0.000    0.000    0.000    0.000 {method 'join' of 'str' objects}
        4    0.000    0.000    0.000    0.000 {built-in method _abc._abc_subclasscheck}
       45    0.000    0.000    0.000    0.000 {method 'add' of 'set' objects}
       10    0.000    0.000    0.000    0.000 {method 'split' of 'str' objects}
        7    0.000    0.000    0.000    0.000 text_block.py:11(__init__)
        1    0.000    0.000    0.000    0.000 codecs.py:309(__init__)
        5    0.000    0.000    0.000    0.000 text_block.py:54(<listcomp>)
        2    0.000    0.000    0.000    0.000 <string>:1(__init__)
        2    0.000    0.000    0.000    0.000 __init__.py:212(_acquireLock)
        5    0.000    0.000    0.000    0.000 _markupbase.py:36(reset)
        5    0.000    0.000    0.000    0.000 component.py:87(__init__)
        2    0.000    0.000    0.000    0.000 god_handler.py:49(process_judgements)
        9    0.000    0.000    0.000    0.000 {method 'popitem' of 'dict' objects}
       10    0.000    0.000    0.000    0.000 __init__.py:122(unescape)
       14    0.000    0.000    0.000    0.000 state.py:17(get_previous)
        1    0.000    0.000    0.000    0.000 <string>:1(__new__)
       20    0.000    0.000    0.000    0.000 {method 'get_ascent' of 'pygame.font.Font' objects}
        7    0.000    0.000    0.000    0.000 chrono.py:135(get_time)
        1    0.000    0.000    0.000    0.000 tokenize.py:380(find_cookie)
        2    0.000    0.000    0.000    0.000 utility.py:147(get_coords_from_shape)
       12    0.000    0.000    0.000    0.000 {method 'endswith' of 'str' objects}
        2    0.000    0.000    0.000    0.000 _internal.py:45(verify_order)
       36    0.000    0.000    0.000    0.000 {built-in method builtins.id}
        1    0.000    0.000    0.000    0.000 pydevd_modify_bytecode.py:195(add_jump_instruction)
        2    0.000    0.000    0.000    0.000 ai.py:34(__init__)
        1    0.000    0.000    0.000    0.000 game_handler.py:23(__init__)
       18    0.000    0.000    0.000    0.000 {method 'end' of 're.Match' objects}
       11    0.000    0.000    0.000    0.000 {method 'group' of 're.Match' objects}
        5    0.000    0.000    0.000    0.000 {method 'sort' of 'list' objects}
        1    0.000    0.000    0.000    0.000 html_parser.py:102(pop_style)
        5    0.000    0.000    0.000    0.000 ui_appearance_theme.py:130(get_font_dictionary)
        4    0.000    0.000    0.000    0.000 component.py:37(__init__)
        5    0.000    0.000    0.000    0.000 event_core.py:46(__init__)
        4    0.000    0.000    0.000    0.000 library.py:206(get_secondary_stat_data)
        3    0.000    0.000    0.000    0.000 component.py:69(__init__)
        2    0.000    0.000    0.000    0.000 {built-in method builtins.setattr}
        4    0.000    0.000    0.000    0.000 manager.py:91(add_ui_element)
        2    0.000    0.000    0.000    0.000 library.py:273(get_god_attitudes_data)
        4    0.000    0.000    0.000    0.000 component.py:61(__init__)
        2    0.000    0.000    0.000    0.000 chrono.py:158(set_turn_holder)
        2    0.000    0.000    0.000    0.000 component.py:188(__init__)
       15    0.000    0.000    0.000    0.000 {method 'items' of 'dict' objects}
        1    0.000    0.000    0.000    0.000 warnings.py:419(__init__)
        2    0.000    0.000    0.000    0.000 __init__.py:221(_releaseLock)
        2    0.000    0.000    0.000    0.000 _internal.py:305(data)
       20    0.000    0.000    0.000    0.000 {built-in method builtins.abs}
        7    0.000    0.000    0.000    0.000 {method 'set_underline' of 'pygame.font.Font' objects}
        3    0.000    0.000    0.000    0.000 chrono.py:105(add_time)
        3    0.000    0.000    0.000    0.000 component.py:138(__init__)
        6    0.000    0.000    0.000    0.000 entity.py:77(<genexpr>)
        2    0.000    0.000    0.000    0.000 combat_stats.py:19(__init__)
        8    0.000    0.000    0.000    0.000 {method 'strip' of 'str' objects}
        4    0.000    0.000    0.000    0.000 ui_window.py:127(get_top_layer)
        2    0.000    0.000    0.000    0.000 __init__.py:1605(getEffectiveLevel)
        5    0.000    0.000    0.000    0.000 manager.py:121(get_gui_manager)
        2    0.000    0.000    0.000    0.000 {built-in method _bisect.bisect_right}
        2    0.000    0.000    0.000    0.000 library.py:248(get_god_intervention_data)
        3    0.000    0.000    0.000    0.000 chrono.py:165(set_time_in_round)
        3    0.000    0.000    0.000    0.000 chrono.py:142(get_time_of_last_turn)
        6    0.000    0.000    0.000    0.000 chrono.py:128(get_time_in_round)
        1    0.000    0.000    0.000    0.000 entity_handler.py:25(__init__)
        1    0.000    0.000    0.000    0.000 god_handler.py:23(__init__)
        2    0.000    0.000    0.000    0.000 {method 'random' of '_random.Random' objects}
        2    0.000    0.000    0.000    0.000 component.py:123(__init__)
        1    0.000    0.000    0.000    0.000 interaction_handler.py:22(__init__)
        4    0.000    0.000    0.000    0.000 {method 'lstrip' of 'str' objects}
        3    0.000    0.000    0.000    0.000 __init__.py:388(<genexpr>)
        1    0.000    0.000    0.000    0.000 pydevd_modify_bytecode.py:82(<lambda>)
        3    0.000    0.000    0.000    0.000 chrono.py:179(set_time_of_last_turn)
        1    0.000    0.000    0.000    0.000 ui_handler.py:27(__init__)
        8    0.000    0.000    0.000    0.000 {method 'setdefault' of 'dict' objects}
        1    0.000    0.000    0.000    0.000 {built-in method builtins.repr}
        2    0.000    0.000    0.000    0.000 component.py:78(__init__)
        1    0.000    0.000    0.000    0.000 chrono.py:172(set_turn_queue)
        1    0.000    0.000    0.000    0.000 codecs.py:260(__init__)
        1    0.000    0.000    0.000    0.000 component.py:180(__init__)
        2    0.000    0.000    0.000    0.000 component.py:97(__init__)
        1    0.000    0.000    0.000    0.000 {method 'seek' of '_io.BufferedReader' objects}
        2    0.000    0.000    0.000    0.000 component.py:115(__init__)
        1    0.000    0.000    0.000    0.000 {method 'search' of 're.Pattern' objects}
        1    0.000    0.000    0.000    0.000 {method 'decode' of 'bytes' objects}
        1    0.000    0.000    0.000    0.000 {method 'startswith' of 'bytes' objects}
        1    0.000    0.000    0.000    0.000 library.py:231(get_god_data)
        3    0.000    0.000    0.000    0.000 {method '__contains__' of 'frozenset' objects}
        3    0.000    0.000    0.000    0.000 {method 'isidentifier' of 'str' objects}
        1    0.000    0.000    0.000    0.000 chrono.py:121(get_turn_queue)
        2    0.000    0.000    0.000    0.000 component.py:106(__init__)
        1    0.000    0.000    0.000    0.000 <string>:1(<module>)
        1    0.000    0.000    0.000    0.000 parser.py:127(clear_cdata_mode)
        1    0.000    0.000    0.000    0.000 {built-in method __new__ of type object at 0x00007FF966139BA0}
        2    0.000    0.000    0.000    0.000 {method 'upper' of 'str' objects}
        1    0.000    0.000    0.000    0.000 {built-in method sys.intern}
        1    0.000    0.000    0.000    0.000 {method 'disable' of '_lsprof.Profiler' objects}


