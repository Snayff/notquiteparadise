...
def _process_opinions(causing_entity: EntityID, reaction_trigger: ReactionTriggerType):
    ...
    for entity, (opinion,) in query.opinion:
        ...
        if reaction_trigger in opinion.attitudes:
            ...
            logging.debug(
                f"{scripts.engine.core.entity.get_name(entity)}`s opinion of {scripts.engine.core.entity.get_name(causing_entity)} is now "
...
def _handle_affliction(entity: EntityID, reaction_trigger: ReactionTriggerType):
    ...
    """
    if scripts.engine.core.entity.entity_has_component(entity, Afflictions):
        afflictions = scripts.engine.core.entity.get_entitys_component(entity, Afflictions)
        for affliction in afflictions.active:
            if reaction_trigger in affliction.triggers:
                scripts.engine.core.entity.trigger_affliction(affliction)
...
def _handle_reaction(observer: EntityID, triggering_entity: EntityID, data: ReactionData):
    ...
    if required_opinion is None or not scripts.engine.core.entity.entity_has_component(observer, Opinion):
        opinion_diff = 0

    else:
        opinion = scripts.engine.core.entity.get_entitys_component(observer, Opinion)
    ...
    if scripts.engine.core.entity.entity_has_component(triggering_entity, Position):
        # get tile of the acting entity
        position = scripts.engine.core.entity.get_entitys_component(triggering_entity, Position)
...
def _apply_reaction(observer: EntityID, triggering_entity: EntityID, target_tile: Tile, data: ReactionData):
    ...
    if isinstance(data.reaction, EffectData):
        effect = scripts.engine.core.entity.create_effect(observer, triggering_entity, data.reaction)
        effect.evaluate()
    else:  # skill data
        from scripts.engine.internal.data import store

        skill = store.skill_registry[data.reaction]
        scripts.engine.core.entity.use_skill(observer, skill, target_tile, Direction.CENTRE)
...
def _set_sprite(entity: EntityID, sprite_category: SpriteCategoryType, new_pos: Optional[Tuple[int, int]] = None):
    ...
    """
    if scripts.engine.core.entity.entity_has_component(entity, Aesthetic):
        aesthetic = scripts.engine.core.entity.get_entitys_component(entity, Aesthetic)
        animation = getattr(aesthetic.sprites, sprite_category)
        if animation:
            aesthetic.set_current_sprite(sprite_category)
        else:
            logging.warning(f"{scripts.engine.core.entity.get_name(entity)} doesn`t have {sprite_category} sprite so left as idle.")
