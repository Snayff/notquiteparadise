...
class DamageEffect(Effect):
    ...
    def evaluate(self) -> Tuple[bool, List[Effect]]:
        ...
        elif not scripts.engine.core.entity.entity_has_component(self.target, Resources):
            logging.info(f"Target doesnt have resources so damage cannot be applied.")
            return False, self.failure_effects

        elif not scripts.engine.core.entity.entity_has_component(self.target, CombatStats):
            logging.warning(f"Target doesnt have combatstats so damage cannot be calculated.")
            return False, self.failure_effects

        elif not scripts.engine.core.entity.entity_has_component(self.origin, CombatStats):
            logging.warning(f"Attacker doesnt have combatstats so damage cannot be calculated.")
            return False, self.failure_effects

        # get combat stats
        defenders_stats = scripts.engine.core.entity.get_entitys_component(self.target, CombatStats)
        attackers_stats = scripts.engine.core.entity.get_entitys_component(self.origin, CombatStats)

        # get hit type
        stat_to_target_value = getattr(defenders_stats, self.stat_to_target.lower())
        to_hit_score = scripts.engine.core.entity.calculate_to_hit_score(attackers_stats.accuracy, self.accuracy, stat_to_target_value)
        hit_type = scripts.engine.core.entity.get_hit_type(to_hit_score)

        # calculate damage
        resist_value = getattr(defenders_stats, "resist_" + self.damage_type.lower())
        mod_value = getattr(attackers_stats, self.mod_stat.lower())
        damage = scripts.engine.core.entity.calculate_damage(self.damage, mod_value, resist_value, hit_type)

        # apply the damage
        if scripts.engine.core.entity.apply_damage(self.target, damage):
            defenders_resources = scripts.engine.core.entity.get_entitys_component(self.target, Resources)
            ...
            if damage >= defenders_resources.health:
                scripts.engine.core.entity.kill_entity(self.target)
...
class MoveSelfEffect(Effect):
    ...
    def evaluate(self) -> Tuple[bool, List[Effect]]:
        ...
        if self.origin != entity:
            logging.debug(f"Failed to move {scripts.engine.core.entity.get_name(entity)} as they are not the originator.")
            return False, self.failure_effects

        # check target has position
        if not scripts.engine.core.entity.entity_has_component(entity, Position):
            logging.debug(f"Failed to move {scripts.engine.core.entity.get_name(entity)} as they have no Position.")
            return False, self.failure_effects
        ...
        for _ in range(0, self.move_amount):
            ...
            if not blocked:
                ...
                _position = scripts.engine.core.entity.get_entitys_component(entity, Position)

                # update position
                if _position:
                    logging.debug(
                        f"->'{scripts.engine.core.entity.get_name(self.target)}' moved from ({pos.x},{pos.y}) to ({new_x}," f"{new_y})."
                    )
...
class MoveOtherEffect(Effect):
    ...
    def evaluate(self) -> Tuple[bool, List[Effect]]:
        ...
        if self.origin == entity:
            logging.debug(f"Failed to move {scripts.engine.core.entity.get_name(entity)} as they are the originator.")
            return False, self.failure_effects
...
class AffectStatEffect(Effect):
    ...
    def evaluate(self) -> Tuple[bool, List[Effect]]:
        ...
        success = False
        stats = scripts.engine.core.entity.get_entitys_component(self.target, CombatStats)
...
class ApplyAfflictionEffect(Effect):
    ...
    def evaluate(self) -> Tuple[bool, List[Effect]]:
        ...
        affliction_instance = scripts.engine.core.entity.create_affliction(affliction_name, origin, target, duration)

        # check for immunities
        if scripts.engine.core.entity.entity_has_immunity(target, affliction_name):
            logging.debug(
                f"'{scripts.engine.core.entity.get_name(self.origin)}' failed to apply {affliction_name} to  "
                f"'{scripts.engine.core.entity.get_name(self.target)}' as they are immune."
            )
            return False, self.failure_effects

        # add the affliction to the afflictions component
        if scripts.engine.core.entity.entity_has_component(target, Afflictions):
            afflictions = scripts.engine.core.entity.get_entitys_component(target, Afflictions)
            afflictions.add(affliction_instance)
            scripts.engine.core.entity.apply_affliction(affliction_instance)

            # add immunities to prevent further applications for the duration
            scripts.engine.core.entity.add_immunity(target, affliction_name, duration + 2)
...
class AffectCooldownEffect(Effect):
    ...
    def evaluate(self) -> Tuple[bool, List[Effect]]:
        ...
        knowledge = scripts.engine.core.entity.get_entitys_component(self.target, Knowledge)
...
class AlterTerrainEffect(Effect):
    ...
    def _create_terrain(self) -> bool:
        ...
        target_pos = scripts.engine.core.entity.get_entitys_component(self.target, Position)
        ...
        if not duplicate:
            ...
            scripts.engine.core.entity.create_terrain(terrain_data, (target_pos.x, target_pos.y), self.affect_amount)
            result = True
