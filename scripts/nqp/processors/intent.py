...
def _process_game_map_intents(intent: InputIntentType):
    ...
    if intent in possible_move_intents and position:
        ...
        move = scripts.engine.core.entity.get_known_skill(player, "Move")
    ...
    ## Use a skill
        ...
        if skill_name:
            ...
            if scripts.engine.core.entity.can_use_skill(player, skill_name):
                skill = scripts.engine.core.entity.get_known_skill(player, skill_name)
    ...
    ## Select new skill
        ...
        if pressed_skill_name:
            ...
            if pressed_skill_name != active_skill_name:
                ...
                if scripts.engine.core.entity.can_use_skill(player, pressed_skill_name):
...
def _process_skill_use(player: EntityID, skill: Type[Skill], target_tile: Tile, direction: DirectionType):
    ...
    if scripts.engine.core.entity.use_skill(player, skill, target_tile, direction):
        scripts.engine.core.entity.pay_resource_cost(player, skill.resource_type, skill.resource_cost)
        scripts.engine.core.entity.set_skill_on_cooldown(player, skill.__class__.__name__, skill.base_cooldown)
...
def _get_pressed_skills_name(intent: InputIntentType) -> Optional[str]:
    ...
    if player:
        skills = scripts.engine.core.entity.get_entitys_component(player, Knowledge)
